...
import AdWaiterModal from '../ads/AdWaiterModal';
import { BouncingDotsLoader } from '../shared/BouncingDotsLoader';
import ReportModal from './ReportModal';
import { getPeople, isItemVisible } from '../../firebase';
...
const EpisodeWatchPage: React.FC<EpisodeWatchPageProps> = ({
    ...
    const selectedEpisode = useMemo(() => {
        ...
        // Block access to upcoming seasons (No admin bypass)
        if (currentSeason.isUpcoming || currentSeason.status === 'coming_soon') return null;

        if (episodeNumber > 0 && episodeNumber <= currentSeason.episodes.length) {
            const ep = currentSeason.episodes[episodeNumber - 1];
            
            // تحقق من الجدولة (No admin bypass)
            if (!isItemVisible(ep)) {
    ...
    if (isBasicLoaded && !selectedEpisode) {
        return (
