...
const ServersManagementTab: React.FC<ServersManagementTabProps> = ({ addToast, onServersChanged }) => {
    ...
    const handleRunMigration = async () => {
        ...
        try {
            ...
            const targetBaseDomainClean = targetBaseDomain.toLowerCase().replace(/^(https?:\/\/)?(www\.)?/, '').replace(/\/$/, '');

            // فلترة الأعمال المرتبطة بهذا السيرفر فقط لتجنب تغيير السيرفرات الأخرى
            const docs = allDocs.filter(doc => {
                const data = doc.data() as Content;
                
                // الحالة 1: العمل مرتبط صراحة بالسيرفر المحدد عبر autoLinkConfig
                if (data.autoLinkConfig?.serverId === migrationServerId) {
                    return true;
                }
                
                // الحالة 2: يحتوي العمل على روابط تتضمن دومين السيرفر المستهدف
                const hasMatchingUrl = (url: string) => {
                    if (!url) return false;
                    const cleanUrl = url.toLowerCase();
                    return cleanUrl.includes(targetBaseDomainClean);
                };

                const hasRootServerMatch = data.servers?.some(srv => hasMatchingUrl(srv.url) || (srv.downloadUrl && hasMatchingUrl(srv.downloadUrl)));
                if (hasRootServerMatch) return true;

                const hasEpisodeServerMatch = data.seasons?.some(season => 
                    season.episodes?.some(ep => 
                        ep.servers?.some(srv => hasMatchingUrl(srv.url) || (srv.downloadUrl && hasMatchingUrl(srv.downloadUrl)))
                    )
                );
                if (hasEpisodeServerMatch) return true;
            ...
            if (docs.length === 0) {
                setCurrentSeriesName('لا توجد أعمال مرتبطة بهذا السيرفر لتحديثها.');
                setMigrating(false);
                return;
            }
            ...
            for (let i = 0; i < docs.length; i++) {
                ...
                try {
                    ...
                    if (!updatedAutoLink && data.type !== 'movie') {
                        updatedAutoLink = {
                        ...
                        needsUpdate = true;
                    } else if (updatedAutoLink && updatedAutoLink.serverId !== migrationServerId) {
                        updatedAutoLink = { ...updatedAutoLink, serverId: migrationServerId };
                        needsUpdate = true;
                    }
                    ...
                    if (needsUpdate) {
                        ...
                        logAction(doc.id, docTitle, 'success', `تم تحويل الروابط وتأكيد ربط العمل بالسيرفر بنجاح`);
                    } else {
                        currentSkipped++;
                        setMigrationSkippedCount(currentSkipped);
                        logAction(doc.id, docTitle, 'skipped', `لا توجد تعديلات مطلوبة (روابط السيرفر محدثة بالفعل)`);
                    }
                } catch (err: any) {
                    currentFailed++;
            ...
            addToast("تم توحيد الروابط للأعمال المتأثرة بنجاح!", "success");
        } catch (err) {
            addToast("فشلت عملية الترحيل والتوحيد.", "error");
        } finally {
            setMigrating(false);
