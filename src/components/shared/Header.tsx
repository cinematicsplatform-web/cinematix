...
const Header: React.FC<HeaderProps> = ({ onSetView, currentUser, activeProfile, onLogout, allContent, onSelectContent, currentView, isRamadanTheme, isEidTheme, isCosmicTealTheme, isNetflixRedTheme, isShahidTheme, returnView, isKidProfile, onOpenSearch, unreadNotificationsCount = 0 }) => {
  const [isScrolled, setIsScrolled] = useState(() => typeof window !== 'undefined' && window.scrollY > 0);
  ...
  const baseMenuItems: { name: string; view: View, loggedIn?: boolean }[] = isShahidTheme
    ? [
  ...
  // Mobile Menu Drawer rendered directly to document.body via Portal to prevent CSS containing block issues on scroll
  const mobileDrawer = (
    <div
      id="mobile-side-menu-drawer-backdrop"
      className={`fixed inset-0 z-[10001] bg-black/70 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${
        isMobileMenuOpen ? 'opacity-100 visible pointer-events-auto' : 'opacity-0 invisible pointer-events-none'
      }`}
      onClick={() => setIsMobileMenuOpen(false)}
    >
      <div 
        id="mobile-side-menu-drawer-panel"
        className={`fixed top-0 right-0 h-full h-[100dvh] w-[290px] max-w-[85vw] bg-[#111622] border-l border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.8)] transition-transform duration-300 ease-out transform overflow-y-auto flex flex-col justify-between ${
          isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        <div>
          <div className="p-5 flex items-center justify-between border-b border-gray-800 bg-[#0d121c]">
            <h2 className="text-xl font-bold">
            {isNetflixRedTheme ? (
               <span className="text-[#E50914] font-['Lalezar'] tracking-wide">CINEMATIX</span>
            ) : isShahidTheme ? (
               <><span className="text-white">سينما</span><span className="text-[#1994e5] font-['Lalezar'] tracking-wide">تيكس</span></>
            ) : (
              ...
              return (
                <a 
                  ...
                  className={`font-bold p-3 rounded-xl transition-all duration-200 text-md flex items-center gap-3 ${
                    isActive 
                      ? (isCosmicTealTheme ? 'bg-[#35F18B]/15 text-[#35F18B]' : isNetflixRedTheme ? 'bg-[#E50914]/15 text-[#E50914]' : isShahidTheme ? 'bg-[#1994e5]/15 text-[#1994e5]' : 'bg-[var(--color-accent)]/15 text-[var(--color-accent)]')
            ...
            {isLoggedIn && (
              <div className="mt-4 pt-4 border-t border-gray-800 flex flex-col gap-2">
                 <div className="px-3 pb-3 mb-2 flex items-center gap-3 border-b border-gray-800/50">
                    <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-gray-700 bg-gray-800 shrink-0">
                      {activeProfile ? <img src={activeProfile.avatar} alt={activeProfile.name} className="w-full h-full object-cover" /> : <UserIcon />}
                    </div>
                    <div className="min-w-0">
                      <p className="font-bold text-sm text-white truncate">{activeProfile?.name}</p>
                 ...
                 <a 
                   href="#" 
                   onClick={(e) => { e.preventDefault(); onSetView('notifications'); setIsMobileMenuOpen(false); }} 
                   className="p-3 rounded-xl flex items-center justify-between text-gray-300 hover:bg-white/5 font-medium transition-colors"
                 >
                    <span>الإشعارات</span>
                 ...
                 <a 
                   href="#" 
                   onClick={(e) => { e.preventDefault(); onSetView('myList'); setIsMobileMenuOpen(false); }} 
                   className="p-3 rounded-xl text-gray-300 hover:bg-white/5 font-medium transition-colors"
                 >
                   قائمتي
                 </a>
                 <a 
                   href="#" 
                   onClick={(e) => { e.preventDefault(); onSetView('accountSettings'); setIsMobileMenuOpen(false); }} 
                   className="p-3 rounded-xl text-gray-300 hover:bg-white/5 font-medium transition-colors"
                 >
                   إدارة الحساب
                 </a>
                 <a 
                   href="#" 
                   onClick={(e) => { e.preventDefault(); onSetView('profileSelector'); setIsMobileMenuOpen(false); }} 
                   className="p-3 rounded-xl text-gray-300 hover:bg-white/5 font-medium transition-colors"
                 >
                   تبديل الملف الشخصي
                 </a>
                 {isAdmin && (
                    <a 
                      href="#" 
                      onClick={(e) => { e.preventDefault(); onSetView('admin'); setIsMobileMenuOpen(false); }} 
                      className="p-3 rounded-xl text-yellow-500 hover:bg-yellow-500/10 font-bold flex items-center justify-between transition-colors"
                    >
                 ...
                 <a 
                   ...
                   className="p-3 rounded-xl text-red-500 hover:bg-red-500/10 mt-2 font-bold flex items-center justify-between transition-colors"
                 >
        ...
        <div className="w-full px-4 md:px-8 flex items-center justify-between h-16 md:h-20 gap-6">
          ...
          <div className="flex items-center gap-6 md:gap-10 lg:flex-1">
            <button 
              ...
              className="lg:hidden text-white p-2 -mr-1 hover:bg-white/10 rounded-lg transition-colors active:scale-95"
