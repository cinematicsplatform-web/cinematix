...
import type { Ad, SiteSettings, User, PinnedContentState, PinnedItem, PageKey, ContentRequest, HomeSection, Content, Top10State, Story, Notification, BroadcastNotification, Person, ReleaseSchedule, GlobalServer, AutoLinkConfig } from '@/types';
...
export const runDatabaseNormalizationMigration = async (serverId: string): Promise<{ successCount: number; failedCount: number }> => {
    ...
    try {
        ...
        console.error("Migration error in src/firebase.ts:", err);
        throw err;
    }
