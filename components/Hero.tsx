...
const Hero: React.FC<HeroProps> = ({ contents, onWatchNow, isLoggedIn, myList, onToggleMyList, autoSlideInterval, isRamadanTheme }) => {
      ...
      <div className={`flex gap-2 pointer-events-none justify-center w-full ${className}`} dir="rtl">
        {contents.map((_, idx) => (
            <button 
                ...
                className={`h-1.5 md:h-2 rounded-full transition-all duration-300 pointer-events-auto cursor-pointer ${currentIndex === idx ? (isRamadanTheme ? 'bg-amber-500 w-6' : 'bg-[#00A7F8] w-6') : 'bg-white/30 hover:bg-white/60 w-2'}`}
                onClick={(e) => { 
  ...
  const renderSlide = (content: Content, position: 'prev' | 'curr' | 'next') => {
      ...
      // UPDATED: Explicit black gradient for Ramadan theme to ensure perfect blend with black body
      ...
      // Use inline style for the custom percentage on mobile, but use md:!object-center to force center on desktop.
      // The !important in Tailwind (!object-center) is crucial to override the inline style on larger screens.
      ...
      return (
        ...
        >
            ...
            */}
            <div className={`absolute inset-0 z-10 ${gradientClass}`}></div>
            ...
            <div className="absolute inset-0 z-30 flex flex-col justify-end px-4 sm:px-12 lg:px-16 pb-8 md:pb-10 text-white pointer-events-none">
                <div className={`max-w-2xl w-full transition-opacity duration-300 pointer-events-auto flex flex-col items-center md:items-start text-center md:text-right ${isActive ? 'opacity-100' : 'opacity-100'}`}>
                    {content.bannerNote && (
                        ...
                        <span className={`px-2 py-0.5 md:px-4 md:py-1.5 rounded-lg text-[10px] md:text-sm font-bold tracking-wide backdrop-blur-md border ${isRamadanTheme ? 'bg-amber-500/20 border-amber-500/50 text-amber-300 shadow-[0_0_20px_rgba(245,158,11,0.3)]' : 'bg-[#00A7F8]/20 border-[#00A7F8]/30 text-[#00A7F8]'}`}>
                            {content.bannerNote}
                        </span>
                        </div>
                    )}
                    ...
                    {content.isLogoEnabled && content.logoUrl ? (
                        ...
                        <h1 className={`text-3xl sm:text-5xl md:text-7xl font-extrabold mb-1 md:mb-4 leading-tight ${isRamadanTheme ? 'text-transparent bg-clip-text bg-gradient-to-b from-amber-100 to-amber-400 drop-shadow-lg' : 'text-white'}`}>
                            {content.title}
                        </h1>
                    )}
                    ...
                    <div className="flex items-center justify-center md:justify-start gap-2 md:gap-6 mb-1 md:mb-5 text-gray-300 text-xs md:text-base font-medium w-full">
                        ...
                        <span className={`border px-1.5 py-0.5 rounded ${isRamadanTheme ? 'border-amber-500/50 text-amber-100' : 'border-gray-500'}`}>{content.ageRating}</span>
        ...
        {hasMultiple && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 hidden md:flex gap-2 pointer-events-none" dir="rtl">
                {contents.map((_, idx) => (
                    <button 
                        key={idx}
                        className={`h-2 rounded-full transition-all duration-300 pointer-events-auto cursor-pointer ${currentIndex === idx ? (isRamadanTheme ? 'bg-amber-500 w-6' : 'bg-[#00A7F8] w-6') : 'bg-white/30 hover:bg-white/60 w-2'}`}
                        onClick={(e) => { 
                            e.stopPropagation(); 
                            if (currentIndex === idx || isAnimating) return;
                            const dir = idx > currentIndex ? 'next' : 'prev';
