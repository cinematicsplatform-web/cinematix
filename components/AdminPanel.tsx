...
const AdminPanel: React.FC<AdminPanelProps> = (props) => {
    ...
    return (
        <div className="bg-[#0f1014] min-h-screen p-4 sm:p-6 lg:p-8 pt-4 md:pt-8 text-white">
            <div className="max-w-8xl mx-auto">
                <div className="flex flex-row justify-between items-center gap-4 mb-8">
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">لوحة التحكم</h1>
                    <button onClick={() => props.onSetView('home')} className="bg-[#1f2937] hover:bg-[#374151] border border-gray-700 font-bold py-2 px-6 rounded-xl transition-all shadow-lg hover:shadow-xl">العودة للموقع</button>
                </div>
                
                {/* Navigation Pills */}
                <div className="mb-8 overflow-x-auto pb-2 rtl-scroll">
                        ...
                        {(['dashboard', 'content', 'pinned', 'users', 'requests', 'ads', 'themes', 'settings', 'analytics', 'notifications'] as AdminTab[]).map(tab => (
                            <button 
                                ...
                                className={`flex-shrink-0 px-6 py-3 rounded-full font-bold text-sm transition-all duration-300 ${
                                    activeTab === tab 
                                    ? 'bg-gradient-to-r from-[#00A7F8] to-[#00FFB0] text-black shadow-[0_0_15px_rgba(0,167,248,0.4)] transform scale-105' 
