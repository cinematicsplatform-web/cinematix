...
type AdminTab = 'dashboard' | 'content' | 'top_content' | 'top10' | 'users' | 'requests' | 'ads' | 'themes' | 'settings' | 'analytics' | 'notifications' | 'stories' | 'app_config' | 'people';
            ...
            <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
                <div className="mb-8 overflow-x-auto py-2 rtl-scroll">
                    <div className="flex gap-2">
                        {(['dashboard', 'content', 'top_content', 'top10', 'stories', 'people', 'users', 'requests', 'ads', 'themes', 'settings', 'analytics', 'notifications', 'app_config'] as AdminTab[]).map(tab => (
                            <button 
                                key={tab} 
                            ...
                            >
                                { {dashboard: 'نظرة عامة', content: 'المحتوى', top_content: 'المحتوى المثبت (Hero)', top10: 'إدارة التوب 10', stories: 'إدارة الستوري', people: 'النجوم والطاقم', users: 'المستخدمون', requests: 'الطلبات', ads: 'إدارة الإعلانات', themes: 'المظهر', settings: 'إعدادات الموقع', analytics: 'الإحصائيات', notifications: 'إرسال إشعار', app_config: 'تطبيق الموبايل'}[tab] }
                            </button>
                        ))}
                    </div>
                </div>
...
const SiteSettingsTab: React.FC<{
    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto">
            ...
            {/* Removed Mobile Application APK Section as requested */}
