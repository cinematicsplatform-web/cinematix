        ...
        <div className="bg-gray-900 min-h-screen p-4 sm:p-6 lg:p-8 pt-4 md:pt-8 text-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-row justify-between items-center gap-4 mb-8">
                    ...
                    <button onClick={() => props.onSetView('home')} className="bg-gray-700 hover:bg-gray-600 font-bold py-2 px-4 rounded-lg transition-colors self-start sm:self-auto">العودة للموقع</button>
                </div>
                <div className="border-b border-gray-700 mb-8"><div className="flex overflow-x-auto rtl-scroll">{(['dashboard', 'content', 'pinned', 'users', 'requests', 'ads', 'themes', 'settings', 'analytics', 'notifications'] as AdminTab[]).map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`flex-shrink-0 px-4 md:px-6 py-3 font-semibold transition-colors ${activeTab === tab ? 'text-[var(--color-accent)] border-b-2 border-[var(--color-accent)]' : 'text-gray-400 hover:text-white'}`}>{ {dashboard: 'نظرة عامة', content: 'المحتوى', pinned: 'المحتوى المثبت', users: 'المستخدمون', requests: 'الطلبات', ads: 'إدارة الإعلانات', themes: 'المظهر (Themes)', settings: 'إعدادات الموقع', analytics: 'الإحصائيات', notifications: 'إرسال إشعار'}[tab] }</button>))}</div></div>
                {renderTabContent()}
            </div>
...
const DashboardTab: React.FC<{stats: {totalMovies: number, totalSeries: number, totalUsers: number}, allContent: Content[]}> = ({stats, allContent}) => {
    ...
    return (<div className="space-y-8 animate-fade-in-up"><div className="grid grid-cols-1 md:grid-cols-3 gap-6"><div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg"><div className="flex items-center justify-between"><h3 className="text-gray-400 text-sm font-medium">إجمالي الأفلام</h3><span className="text-2xl">🎬</span></div><p className="text-4xl font-bold mt-4 text-white">{stats.totalMovies}</p><p className="text-xs text-green-400 mt-2">متاح في قاعدة البيانات</p></div><div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg"><div className="flex items-center justify-between"><h3 className="text-gray-400 text-sm font-medium">إجمالي المسلسلات</h3><span className="text-2xl">📺</span></div><p className="text-4xl font-bold mt-4 text-white">{stats.totalSeries}</p><p className="text-xs text-green-400 mt-2">متاح في قاعدة البيانات</p></div><div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg"><div className="flex items-center justify-between"><h3 className="text-gray-400 text-sm font-medium">المستخدمين المسجلين</h3><span className="text-2xl">👥</span></div><p className="text-4xl font-bold mt-4 text-white">{stats.totalUsers}</p><p className="text-xs text-blue-400 mt-2">حساب نشط</p></div></div><div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden"><div className="px-6 py-4 border-b border-gray-700"><h3 className="font-bold text-lg">أحدث الأنشطة (إضافة/تعديل)</h3></div><div className="overflow-x-auto"><table className="w-full text-sm text-right text-gray-300 whitespace-nowrap"><thead className="bg-gray-700/50 text-xs uppercase"><tr><th className="px-6 py-3">العنوان</th><th className="px-6 py-3">النوع</th><th className="px-6 py-3">تاريخ التعديل</th><th className="px-6 py-3">الحالة</th></tr></thead><tbody>{recentlyAdded.length > 0 ? recentlyAdded.map(item => (<tr key={item.id} className="border-b border-gray-700 hover:bg-gray-700/50"><td className="px-6 py-4 font-medium text-white flex items-center gap-3"><img src={item.poster} alt="" className="w-8 h-10 object-cover rounded"/>{item.title}</td><td className="px-6 py-4">{item.type === 'movie' ? 'فيلم' : 'مسلسل'}</td><td className="px-6 py-4 dir-ltr text-right">{new Date(item.updatedAt || item.createdAt).toLocaleDateString('en-GB')}<span className="text-xs text-gray-500 block">{new Date(item.updatedAt || item.createdAt).toLocaleTimeString('en-US', {hour: '2-digit', minute: '2-digit'})}</span></td><td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs ${item.visibility === 'general' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>{item.visibility === 'general' ? 'عام' : 'مقيد'}</span></td></tr>)) : (<tr><td colSpan={4} className="text-center py-8 text-gray-500">لا يوجد محتوى حديث</td></tr>)}</tbody></table></div></div></div>);
}
...
const ContentManagementTab: React.FC<any> = ({content, onNew, onEdit, onRequestDelete, isLoading, addToast, onBulkSuccess}) => { 
    ...
    const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (!file) return; setProcessingExcel(true); setProgress('جاري قراءة الملف...'); const reader = new FileReader(); reader.onload = async (evt) => { try { const data = new Uint8Array(evt.target?.result as ArrayBuffer); const workbook = XLSX.read(data, { type: 'array' }); if (workbook.Sheets['Movies']) { const movies = XLSX.utils.sheet_to_json<any>(workbook.Sheets['Movies']); let count = 0; const batch = db.batch(); let batchCount = 0; for (const row of movies) { count++; setProgress(`معالجة الفيلم ${count} من ${movies.length}...`); let movieData: any = {}; if (row.TMDB_ID) { const tmdb = await fetchTMDBData(row.TMDB_ID, 'movie'); if (tmdb) { movieData = { title: tmdb.title, description: tmdb.overview, poster: tmdb.poster_path ? `https://image.tmdb.org/t/p/w500${tmdb.poster_path}` : '', backdrop: tmdb.backdrop_path ? `https://image.tmdb.org/t/p/original${tmdb.backdrop_path}` : '', rating: tmdb.vote_average ? Number((tmdb.vote_average / 2).toFixed(1)) : 0, releaseYear: tmdb.release_date ? new Date(tmdb.release_date).getFullYear() : new Date().getFullYear(), genres: tmdb.genres?.map((g: any) => g.name) || [], cast: tmdb.credits?.cast?.slice(0, 5).map((c: any) => c.name) || [] }; } } if (row.Title) movieData.title = row.Title; if (row.Description) movieData.description = row.Description; if (row.Year) movieData.releaseYear = parseInt(row.Year); if (row.Rating) movieData.rating = parseFloat(row.Rating); if (row.Poster_URL) movieData.poster = row.Poster_URL; if (row.Backdrop_URL) movieData.backdrop = row.Backdrop_URL; if (row.Logo_URL) { movieData.logoUrl = row.Logo_URL; movieData.isLogoEnabled = true; } if (row.Genres) movieData.genres = row.Genres.split(',').map((g: string) => g.trim()); const servers: Server[] = []; if (row.Watch_Server_1) servers.push({ id: 1, name: "سيرفر 1", url: row.Watch_Server_1, downloadUrl: "", isActive: true }); if (row.Watch_Server_2) servers.push({ id: 2, name: "سيرفر 2", url: row.Watch_Server_2, downloadUrl: "", isActive: true }); if (row.Watch_Server_3) servers.push({ id: 3, name: "سيرفر 3", url: row.Watch_Server_3, downloadUrl: "", isActive: true }); if (row.Watch_Server_4) servers.push({ id: 4, name: "سيرفر 4", url: row.Watch_Server_4, downloadUrl: "", isActive: true }); if (row.Download_Link) servers.forEach(s => s.downloadUrl = row.Download_Link); const finalMovie: Content = { id: row.TMDB_ID ? String(row.TMDB_ID) : String(Date.now() + Math.random()), type: ContentType.Movie, title: movieData.title || 'New Movie', description: movieData.description || '', poster: movieData.poster || '', backdrop: movieData.backdrop || '', rating: movieData.rating || 0, releaseYear: movieData.releaseYear || new Date().getFullYear(), genres: movieData.genres || [], categories: ['افلام اجنبية'], cast: movieData.cast || [], visibility: 'general', ageRating: '', servers: servers, seasons: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), slug: generateSlug(movieData.title || ''), logoUrl: movieData.logoUrl, isLogoEnabled: movieData.isLogoEnabled }; const ref = db.collection("content").doc(finalMovie.id); batch.set(ref, finalMovie, { merge: true }); batchCount++; if (batchCount >= 400) { await batch.commit(); batchCount = 0; } } if (batchCount > 0) await batch.commit(); } if (workbook.Sheets['Episodes']) { const episodes = XLSX.utils.sheet_to_json<any>(workbook.Sheets['Episodes']); const seriesGroups: Record<string, any[]> = {}; episodes.forEach(ep => { const key = ep.Series_TMDB_ID || ep.Series_Name || 'Unknown'; if (!seriesGroups[key]) seriesGroups[key] = []; seriesGroups[key].push(ep); }); const epBatch = db.batch(); let epBatchCount = 0; let seriesCount = 0; for (const [seriesKey, epRows] of Object.entries(seriesGroups)) { seriesCount++; setProgress(`معالجة المسلسل ${seriesCount} من ${Object.keys(seriesGroups).length}...`); let seriesDoc: any = null; let seriesId = String(seriesKey); const existingSeries = content.find((c:any) => c.id === seriesId || c.title === seriesKey); if (existingSeries) { seriesDoc = { ...existingSeries }; seriesId = existingSeries.id; } else { let tmdbSeries: any = null; if (!isNaN(Number(seriesKey))) { tmdbSeries = await fetchTMDBData(seriesKey, 'tv'); } seriesDoc = { id: seriesId, type: ContentType.Series, title: tmdbSeries?.name || epRows[0].Series_Name || 'New Series', description: tmdbSeries?.overview || '', poster: tmdbSeries?.poster_path ? `https://image.tmdb.org/t/p/w500${tmdbSeries.poster_path}` : '', backdrop: tmdbSeries?.backdrop_path ? `https://image.tmdb.org/t/p/original${tmdbSeries.backdrop_path}` : '', rating: tmdbSeries?.vote_average ? Number((tmdbSeries.vote_average / 2).toFixed(1)) : 0, releaseYear: tmdbSeries?.first_air_date ? new Date(tmdbSeries.first_air_date).getFullYear() : new Date().getFullYear(), genres: tmdbSeries?.genres?.map((g: any) => g.name) || [], categories: ['مسلسلات اجنبية'], seasons: [], visibility: 'general', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), slug: generateSlug(tmdbSeries?.name || epRows[0].Series_Name || '') }; } if (!seriesDoc.seasons) seriesDoc.seasons = []; for (const ep of epRows) { const sNum = parseInt(ep.Season_Number) || 1; const eNum = parseInt(ep.Episode_Number) || 1; let season = seriesDoc.seasons.find((s: Season) => s.seasonNumber === sNum); if (!season) { season = { id: Date.now() + Math.random(), seasonNumber: sNum, title: `الموسم ${sNum}`, episodes: [] }; seriesDoc.seasons.push(season); } const episodeObj: Episode = { id: Date.now() + Math.random(), title: ep.Episode_Title || `الحلقة ${eNum}`, thumbnail: seriesDoc.backdrop || '', duration: 45, progress: 0, servers: [] }; if (ep.Watch_Server_1) episodeObj.servers.push({ id: 1, name: "سيرفر 1", url: ep.Watch_Server_1, downloadUrl: ep.Download_Link || "", isActive: true }); if (ep.Watch_Server_2) episodeObj.servers.push({ id: 2, name: "سيرفر 2", url: ep.Watch_Server_2, downloadUrl: "", isActive: true }); const existingEpIndex = season.episodes.findIndex((e: Episode) => e.title?.includes(`${eNum}`) || e.title === ep.Episode_Title); if (existingEpIndex > -1) { season.episodes[existingEpIndex] = { ...season.episodes[existingEpIndex], ...episodeObj, servers: [...season.episodes[existingEpIndex].servers, ...episodeObj.servers] }; } else { season.episodes.push(episodeObj); } } seriesDoc.seasons.sort((a: Season, b: Season) => a.seasonNumber - b.seasonNumber); seriesDoc.seasons.forEach((s: Season) => { s.episodes.sort((a: Episode, b: Episode) => { const numA = parseInt(a.title?.replace(/\D/g, '') || '0'); const numB = parseInt(b.title?.replace(/\D/g, '') || '0'); return numA - numB; }); }); const ref = db.collection("content").doc(seriesDoc.id); epBatch.set(ref, seriesDoc, { merge: true }); epBatchCount++; if (epBatchCount >= 300) { await epBatch.commit(); epBatchCount = 0; } } if (epBatchCount > 0) await epBatch.commit(); } addToast('تم استيراد البيانات من Excel بنجاح!', 'success'); onBulkSuccess(); } catch (err) { console.error("Excel Import Error:", err); addToast('حدث خطأ أثناء معالجة ملف Excel.', 'error'); } finally { setProcessingExcel(false); setProgress(''); if (excelInputRef.current) excelInputRef.current.value = ''; } }; reader.readAsArrayBuffer(file); }; 
    ...
    return (
        <div>
            ...
            {processingExcel && (<div className="mb-6 bg-gray-800 p-4 rounded-xl border border-gray-700 animate-pulse"><div className="flex justify-between mb-2 text-sm text-blue-400 font-bold"><span>جاري الاستيراد...</span><span>{progress}</span></div><div className="w-full bg-gray-700 rounded-full h-2.5"><div className="bg-blue-600 h-2.5 rounded-full w-2/3 transition-all duration-500"></div></div><p className="text-xs text-gray-500 mt-2 text-center">الرجاء عدم إغلاق الصفحة حتى تكتمل العملية.</p></div>)}
            
            {/* CONTENT DISPLAY AREA */}
            {isLoading ? (
                <div className="text-center py-20 text-gray-400">جاري تحميل المحتوى من قاعدة البيانات...</div> 
            ) : (
                <>
                    {filteredContent.length === 0 && (
                        <div className="text-center py-10 text-gray-500 border border-dashed border-gray-700 rounded-xl mb-4">لا يوجد محتوى مطابق للبحث.</div>
                    )}

                    {/* MOBILE CARD VIEW (Visible only on mobile) */}
                    <div className="md:hidden space-y-4">
                        {filteredContent.map((c:any) => (
                            <div key={c.id} className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-sm relative overflow-hidden">
                                {/* Header: Poster + Title + Type */}
                                <div className="flex gap-3 mb-3">
                                    <div className="w-16 h-24 flex-shrink-0 rounded-lg overflow-hidden bg-gray-900 border border-gray-600">
                                        <img src={c.poster} alt={c.title} className="w-full h-full object-cover" loading="lazy" />
                                    </div>
                                    <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
                                        <h4 className="font-bold text-white text-base leading-tight line-clamp-2">{c.title}</h4>
                                        <div className="flex items-center gap-2 text-xs">
                                            <span className={`px-2 py-0.5 rounded font-bold ${c.type === 'movie' ? 'bg-blue-900/30 text-blue-400 border border-blue-500/20' : 'bg-purple-900/30 text-purple-400 border border-purple-500/20'}`}>
                                                {c.type === 'movie' ? 'فيلم' : 'مسلسل'}
                                            </span>
                                            <span className="text-gray-400">{c.releaseYear}</span>
                                        </div>
                                    </div>
                                </div>

                                {/* Details Row */}
                                <div className="flex items-center justify-between py-3 border-t border-gray-700/50 text-xs text-gray-400">
                                    <div className="flex items-center gap-1">
                                        <span>📅 التحديث:</span>
                                        <span dir="ltr" className="text-gray-300">{new Date(c.updatedAt || c.createdAt).toLocaleDateString('en-GB')}</span>
                                    </div>
                                    <div className="flex items-center gap-1">
                                        <span>👁️ الحالة:</span>
                                ...
                                <div className="flex gap-3 pt-1">
                                    <button 
                                        onClick={() => onEdit(c)} 
                                        className="flex-1 bg-green-600/10 hover:bg-green-600/20 text-green-400 border border-green-600/20 py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
                                    >
                                        ✏️ تعديل
                                    </button>
                                    <button 
                                        onClick={() => onRequestDelete(c.id, c.title)} 
                                        className="flex-1 bg-red-600/10 hover:bg-red-600/20 text-red-400 border border-red-600/20 py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
                                    >
                                        🗑️ حذف
                                    </button>
                                </div>
                                ...
                                {filteredContent.map((c:any) => (
                                    <tr key={c.id} className="border-b border-gray-700 hover:bg-gray-700/50 transition-colors">
                                        <td className="px-6 py-4 font-medium text-white flex items-center gap-3">
                                            <img src={c.poster} alt={c.title} className="w-10 h-14 object-cover rounded bg-gray-900 border border-gray-600"/>
                                            <span>{c.title}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded text-xs ${c.type === 'movie' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>
                                                {c.type === 'movie' ? 'فيلم' : 'مسلسل'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded text-xs ${c.visibility === 'general' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                                                {c.visibility === 'general' ? 'عام' : 'مقيد'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 space-x-2 space-x-reverse">
                                            <button onClick={() => onEdit(c)} className="font-medium text-green-400 hover:text-green-300 hover:underline transition-colors">تعديل</button>
                                            <button onClick={() => onRequestDelete(c.id, c.title)} className="font-medium text-red-400 hover:text-red-300 hover:underline transition-colors">حذف</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
        ...
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="bg-gray-800 p-6 rounded-xl border border-gray-700">
                ...
                <div className="bg-gray-700/30 p-4 rounded-lg border border-gray-600 mb-4">
                    ...
                    <p className="text-xs text-gray-400 mb-4">قم بتنزيل الملفات التالية ورفعها إلى مجلد `public` في مشروعك لضمان الفهرسة الكاملة في جوجل.</p>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                        <button onClick={() => generateSpecificSitemap('index')} className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
                            <DocumentArrowDownIcon /> 1. Sitemap Index
                        </button>
                        <button onClick={() => generateSpecificSitemap('movies')} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
                            <DocumentArrowDownIcon /> 2. Movies XML
                        </button>
                        <button onClick={() => generateSpecificSitemap('series')} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
                            <DocumentArrowDownIcon /> 3. Series XML
                        </button>
                        <button onClick={() => generateSpecificSitemap('seasons')} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
                            <DocumentArrowDownIcon /> 4. Seasons XML
                        </button>
                        <button onClick={() => generateSpecificSitemap('episodes')} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg text-sm transition-colors flex items-center justify-center gap-2">
...
const NotificationTab: React.FC<{ addToast: AdminPanelProps['addToast'], serviceAccountJson?: string }> = ({ addToast, serviceAccountJson }) => {
    const [title, setTitle] = useState(''); const [body, setBody] = useState(''); const [image, setImage] = useState(''); const [url, setUrl] = useState(''); const [isSending, setIsSending] = useState(false); const handleSend = async (e: React.FormEvent) => { e.preventDefault(); if (!title || !body) { addToast('الرجاء تعبئة العنوان والرسالة.', 'error'); return; } if (!serviceAccountJson) { addToast('يجب إدخال Service Account JSON في الإعدادات أولاً.', 'error'); return; } setIsSending(true); try { const accessToken = await getAccessToken(serviceAccountJson); if (!accessToken) { addToast('فشل في إنشاء رمز الوصول (Access Token). تأكد من صحة ملف JSON.', 'error'); setIsSending(false); return; } const parsedServiceAccount = JSON.parse(serviceAccountJson); const projectId = parsedServiceAccount.project_id; const usersSnapshot = await db.collection('users').get(); const tokens: string[] = []; usersSnapshot.forEach(doc => { const userData = doc.data(); if (userData.fcmTokens && Array.isArray(userData.fcmTokens)) { userData.fcmTokens.forEach((token: string) => { if (token && !tokens.includes(token)) { tokens.push(token); } }); } }); if (tokens.length === 0) { addToast('لم يتم العثور على أي مستخدمين مسجلين للإشعارات.', 'info'); setIsSending(false); return; } console.log(`Sending notification to ${tokens.length} devices...`); const results = await Promise.all(tokens.map(async (token) => { try { const notification = { title, body, image: image || undefined, data: { url: url || '/' } }; await sendFCMv1Message(token, notification, accessToken, projectId); return true; } catch (error) { console.error('FCM Error:', error); return false; } })); const successCount = results.filter(Boolean).length; if (successCount > 0) { addToast(`تم إرسال الإشعار بنجاح إلى ${successCount} جهاز!`, 'success'); setTitle(''); setBody(''); setImage(''); setUrl(''); } else { addToast('فشل إرسال الإشعار لجميع الأجهزة.', 'error'); } } catch (error) { console.error(error); addToast('حدث خطأ غير متوقع.', 'error'); } finally { setIsSending(false); } }; return (<div className="max-w-2xl mx-auto bg-gray-800 p-8 rounded-xl border border-gray-700 animate-fade-in-up"><div className="flex items-center gap-3 mb-6 border-b border-gray-700 pb-4"><PaperAirplaneIcon /><h3 className="text-xl font-bold text-white">إرسال إشعار للمستخدمين</h3></div>{!serviceAccountJson && (<div className="mb-6 bg-red-900/20 border border-red-500/30 p-4 rounded-xl text-red-200 text-sm">⚠️ لم يتم إعداد <strong>Service Account JSON</strong>. يرجى إضافته من تبويب "إعدادات الموقع" لتفعيل الإرسال.</div>)}<form onSubmit={handleSend} className="space-y-5"><div><label className="block text-sm font-bold text-gray-400 mb-2">عنوان الإشعار</label><input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] text-white" placeholder="مثال: نزل الآن!" required/></div><div><label className="block text-sm font-bold text-gray-400 mb-2">نص الرسالة</label><textarea value={body} onChange={e => setBody(e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] text-white h-24 resize-none" placeholder="مثال: الحلقة 5 من المؤسس عثمان متاحة بجودة عالية" required/></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-bold text-gray-400 mb-2">رابط الصورة (اختياري)</label><input type="text" value={image} onChange={e => setImage(e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] text-white dir-ltr" placeholder="https://..."/></div><div><label className="block text-sm font-bold text-gray-400 mb-2">رابط التوجيه (Deep Link)</label><input type="text" value={url} onChange={e => setUrl(e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] text-white dir-ltr" placeholder="/movie/inception"/><p className="text-[10px] text-gray-500 mt-1">المسار النسبي للفيلم أو المسلسل (مثال: /series/game-of-thrones)</p></div></div><div className="pt-4"><button type="submit" disabled={isSending || !serviceAccountJson} className={`w-full bg-gradient-to-r from-[var(--color-primary-from)] to-[var(--color-primary-to)] text-black font-bold py-3 px-6 rounded-lg transition-all transform hover:scale-[1.02] flex items-center justify-center gap-2 ${isSending || !serviceAccountJson ? 'opacity-70 cursor-not-allowed' : 'hover:shadow-lg'}`}>{isSending ? (<><div className="w-5 h-5 border-2 border-black border-t-transparent rounded-full animate-spin"></div><span>جاري الإرسال...</span></>) : (<><PaperAirplaneIcon /><span>إرسال الإشعار الآن</span></>)}</button></div></form></div>);
};
